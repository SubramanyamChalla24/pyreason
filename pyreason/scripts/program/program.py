...
class Program:
	...
	def __init__(self, graph, tmax, facts_node, facts_edge, rules, ipl, reverse_graph, atom_trace, save_graph_attributes_to_rule_trace, canonical, inconsistency_check, ad_hoc_grounding, resolution_levels, step_size):
		self._graph = graph
		self._tmax = tmax
	...
	def reason(self, convergence_threshold, convergence_bound_threshold, verbose=True):
		...
		interp = Interpretation(self._graph, self._tmax, self._ipl, self._reverse_graph, self._atom_trace, self._save_graph_attributes_to_rule_trace, self._canonical, self._inconsistency_check, self._ad_hoc_grounding, self._resolution_levels, self._step_size, convergence_threshold, convergence_bound_threshold)
